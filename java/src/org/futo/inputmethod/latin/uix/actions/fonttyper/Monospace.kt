...
object MonospaceRenderer : WordImageRenderer() {
    override val name: Int
        get() = R.string.action_fonttyper_preset_title_monospace

    override fun renderLine(
        context: Context,
        text: String
    ): LineRenderResult? {
        val textColor = 0xFF00FF00.toInt()
        ...
        // Draw terminal-like background
        canvas.drawColor(backgroundColor)

        val xOffs = 20f
        val yOffs = (bounds.height().toFloat() + 40f) / 2.0f + 10f

        // Draw text with terminal style
...
private fun PreviewRenderer() {
    ...
    val renderer = MonospaceRenderer
    val image = remember {
